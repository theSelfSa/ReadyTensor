"""
Scoring engines - replace LangChain with direct OpenAI API calls
Preserve all original assessment logic from directory_scorer/
"""
...
class ContentBasedScorer:
    """Content-based scoring - replace LangChain document processing"""
    ...
    def assess_repository(self, repo_path: str) -> Dict[str, Any]:
        """Assess repository content - preserve original content_based_scorer.py logic"""
        
        # Get files to assess (same filtering as original)
        files_to_assess = self._get_assessable_files(repo_path)
        logger.info(f"Assessing {len(files_to_assess)} files")
        
        results = {}
        
        # Process files in parallel (same as original)
    ...
    def _get_assessable_files(self, repo_path: str) -> List[str]:
        """Get files for assessment - same logic as original"""
        extensions = ['.py', '.md', '.ipynb', '.txt']
        files = []
        
        for root, dirs, filenames in os.walk(repo_path):
            # Skip directories (same as original)
            ...
            for filename in filenames:
                if any(filename.endswith(ext) for ext in extensions):
                    ...
                    # Skip files that are too large (same as original)
    ...
    def _assess_file(self, file_path: str) -> Dict[str, Any]:
        """Assess individual file - replace LangChain with direct API"""
        
        try:
            # Load file content (replace LangChain document loader)
            content = self._load_file_content(file_path)
            
            if not content.strip():
                return {"score": 0, "explanation": "Empty file"}
            
            # Truncate if too long (same logic as original)
            if self.llm_client.count_tokens(content) > self.token_limit:
                content = self._truncate_content(content)
            
            # Create assessment prompt (same as original prompts)
            prompt = self._create_assessment_prompt(content, file_path)
            
            # Get LLM assessment (replace LangChain invoke with direct API)
            response = self.llm_client.invoke_structured(prompt, CriterionResponse)
            
            logger.info(f"Assessed {os.path.basename(file_path)}: {response.get('score', 0)}")
            return response
    ...
    def _truncate_content(self, content: str) -> str:
        ...
        # Simple truncation (same approach as original)
        while self.llm_client.count_tokens(content) > self.token_limit:
